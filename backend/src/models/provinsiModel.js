...
const createProvinsi = async (nama) => {
    const result = await pool.query('INSERT INTO provinsi (nama) VALUES ($1) RETURNING *', [nama]);
    return result.rows[0];
};

const updateProvinsi = async (id, nama) => {
    const result = await pool.query('UPDATE provinsi SET nama = $1 WHERE id = $2 RETURNING *', [nama, id]);
    return result.rows[0];
};
...
module.exports = { getAllProvinsi, getProvinsiById, createProvinsi, updateProvinsi, deleteProvinsi };